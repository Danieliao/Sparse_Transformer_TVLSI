...
the activations are stationary in Input Memory and the weights stream through the chain; the stationary tile dimension is the $M{\cdot}P$ output rows produced in parallel, and the column dimension of the output is iterated address-by-address across Accumulator Memory. In the \emph{column-wise} mode, the roles are flipped: weights are loaded stationary into Input Memory (the same SRAM bank, written through the same RRLC decoder), activations are fed to Weight Memory and their Run goes to the pointer Memory. As activations stream through the chain, and the stationary tile dimension becomes the $M{\cdot}P$ output \emph{columns} produced in parallel. Note that multiple columns of Input and Weight are compressed to RRLC format instead of multiple rows in row-wise mode.
